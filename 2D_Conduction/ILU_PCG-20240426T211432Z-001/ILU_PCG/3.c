...
//----------------------Jacobi Preconditioner-----------------------
    ...
    for(int i =1;i<N;i++)
    {
        Ax[i] = A_n[i]*T[i-n] + A_s[i]*T[i-1] + A_p[i]*T[i] + A_n[i]*T[i+1] + A_e[i]*T[i+n]; 
        r[i] = b[i] - Ax[i];
        //d[i] = r[i]/M[i]; //----------------M^-1--------------
    }
...
for(int i =1;i<N;i++)
    {
        Ad[i] = A_n[i]*d[i-n] + A_s[i]*d[i-1] + A_p[i]*d[i] + A_n[i]*d[i+1] + A_e[i]*d[i+n];
         //---------------M^-1--------------
    }
...
void M_inverse(int N,int n,double M_inverse_r[N],double A_p[N],double A_n[N],double A_w[N],double A_e[N],double A_s[N],double r[N])
{
double L_s[N],L_w[N],L_p[N],U_e[N],U_n[N],R[N];
FILE *fp1;
fp1 = fopen("ILU.csv","w");
...
for(int i=1; i < N;i++)
	...
	L_w[i] = A_w[i];
	if(i == 1) L_p[i] = A_p[i];
	else if( i <= n) L_p[i] = A_p[i] - (L_s[i] * U_n[i-1]); 
		else L_p[i] = A_p[i] - (L_s[i] * U_n[i-1]) - (L_w[i] * U_e[i-n]);
	U_e[i] = A_e[i] / L_p[i];
	U_n[i] = A_n[i] / L_p[i];
	}
...
if(i == 1) R[i] = r[i] / L_p[i] ;
	else if( i <= n) R[i] = (r[i] - (L_s[i] * R[i-1])) / L_p[i]; 
		else R[i] = (r[i] - (L_s[i] * R[i-1]) - (L_w[i] * R[i-n]))/L_p[i];
}

for(int i= N-1; i >= 1;i--)
{
if(i == N-1) M_inverse_r[i] = R[i];
	else if( i >= N-n-1) M_inverse_r[i] = R[i] - (U_n[i] * M_inverse_r[i+1]); 
		else M_inverse_r[i] = R[i] - (U_n[i] * M_inverse_r[i+1]) - (U_e[i] * M_inverse_r[i+n]);


}
fprintf(fp1,"L_p\t L_s \t L_w \t U_e \t U_n \t R \t delta\n");
  for(int i = 1; i< N ; i++)fprintf(fp1,"%.3lf\t %.3lf \t %.3lf \t %.3lf \t %.3lf \t %.3lf \t %.3lf\n",L_p[i],L_s[i],L_w[i],U_e[i],U_n[i],R[i],M_inverse_r[i]);
